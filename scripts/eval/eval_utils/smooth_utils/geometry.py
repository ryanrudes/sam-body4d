...
def axis_angle_to_rotation_matrix(axis_angle: torch.Tensor) -> torch.Tensor:
    ...
    return R.reshape(*orig_shape[:-1], 3, 3)
...
def rotation_matrix_to_axis_angle(R: torch.Tensor) -> torch.Tensor:
    ...
    axis = axis / (2.0 * torch.sin(theta).unsqueeze(1) + 1e-8)
    aa = axis * theta.unsqueeze(1)

    aa[theta < 1e-6] = 0.0

    return aa.reshape(*orig_shape[:-2], 3)


def rotation_matrix_to_rot6d(R: torch.Tensor) -> torch.Tensor:
    """
    R: (..., 3, 3)
    return: (..., 6)
    """
...
def smooth_with_slerp(input: torch.Tensor, sigma: float = 1.0) -> torch.Tensor:
    def fix_time_continuity(q: torch.Tensor, time_dim: int = -3):
        shape = q.shape
