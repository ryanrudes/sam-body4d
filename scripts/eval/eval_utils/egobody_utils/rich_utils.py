...
def get_cam2params(root=None, compute=True):
    """
    Args:
    ...
    if compute:
        cam_params = {}
        cam_xml_files = (root / "scan_calibration").glob("*/calibration/*.xml")
        ...
        cam_params = torch.load(root / "resource/cam2params.pt")
    return cam_params
...
def parse_seqname_info(root, skip_multi_persons=True):
    """
    This function will skip multi-person sequences.
    Returns:
        sname_to_info: scan_name, subject_id, gender, cam_ids
    """
    fns = [root / f"resource/{split}.txt" for split in ["train", "val", "test"]]
...
def get_seqnames_of_split(root, splits=["train"], skip_multi_persons=True):
    if not isinstance(splits, list):
        splits = [splits]
    fns = [root / f"resource/{split}.txt" for split in splits]
    seqnames = []
    for fn in fns:
        with open(fn, "r") as f:
            for line in f.readlines()[1:]:
